...
// Build a map: url -> sorted array of {score, createdAt} oldest-first
...
// Returns trend delta vs previous scan of same URL, or null if first
...
export default function Dashboard() {
  ...
  useEffect(() => { if (!isPending && !session) navigate('/sign-in'); }, [session, isPending, navigate]);
  useEffect(() => {
    if (!session) return;
  ...
  const handleCancel = async () => {
    setCancelling(true);
    try { await cancelSubscription(); const fresh = await getSubscription(); setSub(fresh); setShowCancelConfirm(false); }
    catch (err) { alert(err?.response?.data?.error || 'Failed to cancel. Please try again.'); }
    finally { setCancelling(false); }
  };

  function avgScore(arr) { if (!arr.length) return 0; return Math.round(arr.reduce((a, s) => a + (s.score ?? 0), 0) / arr.length); }
  function totalBugs(arr) { return arr.reduce((a, s) => a + (s.violations ?? 0), 0); }
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="pixel-box p-6 flex flex-col sm:flex-row items-start sm:items-center gap-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            {plan !== 'FREE' && <button onClick={handlePortal} disabled={portalLoading} className="btn-ghost text-xs py-2 px-4">{portalLoading ? '...' : '⚙ MANAGE PLAN'}</button>}
        ...
        {isPaidPlan && (
          <div className="pixel-box p-4 space-y-3">
            ...
            {!cancelAtEnd && (
              <div className="pt-1">
                {!showCancelConfirm ? (
                  <button onClick={() => setShowCancelConfirm(true)} className="text-xs text-pixel-red border border-pixel-red px-3 py-1 hover:bg-pixel-red hover:text-pixel-dark transition-colors">❌ CANCEL SUBSCRIPTION</button>
                ) : (
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <div className="pixel-box py-2 inline-flex items-center gap-2"><span className="text-pixel-green text-xs">📜 QUEST LOG</span></div>
            <Link to="/scan" className="text-pixel-green text-xs hover:text-glow-green">+ NEW QUEST</Link>
          </div>
          {loadingSub ? <PixelLoader message="LOADING HISTORY..." /> : scans.length === 0 ? (
            ...
            <div className="space-y-3">
              {scans.slice(0, 10).map((scan) => <ScanRow key={scan.id} scan={scan} trend={getTrend(scan, trendMap)} />)}
            </div>
          )}
        </div>
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-3 flex-shrink-0 items-center">
        ...
        {scan.id && (
          <button onClick={handlePdfDownload} className="text-pixel-cyan text-xs hover:underline cursor-pointer">📄 PDF</button>
        )}
