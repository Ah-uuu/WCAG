...
const SCAN_STAGES = [
  { label:'ENTERING DUNGEON',      target:18 },
  { label:'SCOUTING THE PAGE',     target:38 },
  { label:'BATTLING THE DOM TREE', target:62 },
  { label:'READING WCAG SCROLLS',  target:82 },
...
function injectStyle(id, css) {
  ...
  const e = document.createElement('style'); e.id = id; e.textContent = css;
...
function ScanProgressBar({ progress, stageIdx, done }) {
  ...
  return (
    <div style={{ background:'#0a0a0f', border:'2px solid #ffd700', padding:24, fontFamily:'monospace' }}>
      ...
      <div style={{ width:'100%', height:22, background:'#111118', border:'1px solid #333', position:'relative', overflow:'hidden', marginBottom:14 }}>
        <div style={{ width:progress+'%', height:'100%', background:'linear-gradient(90deg,'+c+'cc,'+c+')', transition:'width .3s cubic-bezier(.4,0,.2,1)', position:'relative' }}
             className={progress>5 && !done ? 'scan-shimmer' : ''} />
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
        {SCAN_STAGES.map((st, i) => {
          const d = done || i < stageIdx; const cur = !done && i === stageIdx;
          return (
...
// ── 訪客掃描上限提示牆（第二次嘗試掃描時顯示）────────────────────────────────
function GuestLimitWall({ onNavigate }) {
  return (
    <div style={{ background:'#0a0a0f', border:'2px solid #ffd700', padding:32, textAlign:'center', fontFamily:'monospace' }}>
      <div style={{ fontSize:40, marginBottom:16 }}>⚔️</div>
      <div style={{ color:'#ffd700', fontWeight:'bold', fontSize:14, letterSpacing:2, marginBottom:12 }}>
        FREE SCAN USED
      </div>
      <div style={{ color:'#aaa', fontSize:12, marginBottom:8, lineHeight:1.8 }}>
        你已使用本次免費掃描額度。
      </div>
      <div style={{ color:'#666', fontSize:11, marginBottom:24, lineHeight:1.8 }}>
        註冊帳號完全免費，無掃描次數限制。
      </div>
      <div style={{ display:'flex', gap:12, justifyContent:'center', flexWrap:'wrap' }}>
        <button
          onClick={() => onNavigate('/sign-up')}
          style={{ background:'#22c55e', color:'#0a0a0f', border:'none', padding:'10px 28px', fontFamily:'monospace', fontWeight:'bold', cursor:'pointer', fontSize:12, letterSpacing:1 }}
        >
          🎮 免費註冊
        </button>
        <button
          onClick={() => onNavigate('/sign-in')}
          style={{ background:'transparent', color:'#ffd700', border:'1px solid #ffd700', padding:'10px 28px', fontFamily:'monospace', fontWeight:'bold', cursor:'pointer', fontSize:12, letterSpacing:1 }}
        >
          🔑 已有帳號？登入
        </button>
      </div>
...
// ── 訪客「0 次剩餘」橫幅（第一次掃描完畢後顯示）──────────────────────────────
function GuestUsedBanner({ onNavigate }) {
  return (
    <div style={{ background:'#1a1500', border:'1px solid #ffd700', padding:'12px 16px', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:8, fontFamily:'monospace' }}>
      <span style={{ color:'#ffd700', fontSize:11 }}>
        ⚠ 免費掃描額度已用完（0 次剩餘）— 註冊帳號享無限掃描
      </span>
      <div style={{ display:'flex', gap:8 }}>
        <button
          onClick={() => onNavigate('/sign-up')}
          style={{ background:'#22c55e', color:'#0a0a0f', border:'none', padding:'5px 14px', fontFamily:'monospace', fontWeight:'bold', cursor:'pointer', fontSize:11 }}
        >免費註冊</button>
        <button
          onClick={() => onNavigate('/sign-in')}
          style={{ background:'transparent', color:'#ffd700', border:'1px solid #ffd700', padding:'5px 14px', fontFamily:'monospace', cursor:'pointer', fontSize:11 }}
        >登入</button>
      </div>
    </div>
  );
...
export default function Scan() {
  ...
  const handleScan = async (e) => {
    ...
    try {
      ...
      // 訪客：標記已使用免費掃描
      if (!isLoggedIn) {
        try { localStorage.setItem('guestScanned', '1'); } catch {}
      }
    } catch (err) {
      apiDoneRef.current = true;
      if (err.response?.status === 403 && err.response?.data?.error === 'GUEST_LIMIT_REACHED') {
  ...
  const handleRepair = async () => {
    ...
    try {
      ...
      setRepairError(err.response?.data?.error || 'Auto-fix failed.');
    } finally {
      setRepairing(false);
    }
  };
  ...
  const downloadRepaired = () => {
    ...
    a.href = URL.createObjectURL(blob); a.download = 'repaired.html'; a.click();
    URL.revokeObjectURL(a.href);
  };

  const scoreColor = s => s >= 90 ? 'text-pixel-green' : s >= 70 ? 'text-pixel-yellow' : s >= 50 ? 'text-orange-400' : 'text-pixel-red';

  // 是否為已用完額度的訪客（localStorage 標記）
  const guestAlreadyScanned = !isLoggedIn && (() => { try { return localStorage.getItem('guestScanned') === '1'; } catch { return false; } })();
  ...
  return (
    <div className="min-h-screen bg-pixel-bg py-8 px-4">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {/* 訪客第二次嘗試掃描 → 顯示限制牆 */}
        {guestLimitReached && <GuestLimitWall onNavigate={navigate} />}
        ...
        {result && (
          <div className="space-y-5">
            ...
            {/* 訪客第一次掃描完畢 → 顯示「0 次剩餘」橫幅 */}
            {!isLoggedIn && <GuestUsedBanner onNavigate={navigate} />}
              ...
              {result.summary && (
                <div className="grid grid-cols-4 gap-2">
                  {Object.entries(result.summary).map(([k, v]) => {
                    const c = SUMMARY_LABELS[k] || { label:k.toUpperCase(), color:'text-pixel-gray' };
                    return (
                      <div key={k} className="pixel-box-dark p-3 text-center">
                        <div className={`font-pixel text-lg ${c.color}`}>{v}</div>
                        <div className="text-pixel-gray text-xs mt-1">{c.label}</div>
                      </div>
                    );
                  })}
                </div>
            ...
            {repair && (
              <div className="pixel-box p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button onClick={downloadRepaired} className="btn-green text-xs py-1 px-3">⬇ Download Fixed HTML</button>
                </div>
                ...
                {repair.fixesApplied?.length > 0 && (
                  <ul className="space-y-2 mb-4">
                    {repair.fixesApplied.map((f, i) => (
                      <li key={i} className="flex items-start gap-2 text-xs text-pixel-white">
                        <span className="text-pixel-green mt-0.5">✓</span><span>{f}</span>
                      </li>
                    ))}
                  </ul>
                )}
                {repair.diff && (
                  <details>
                    <summary className="cursor-pointer text-xs text-pixel-gray hover:text-pixel-white font-pixel">▶ VIEW DIFF</summary>
                    <pre className="mt-2 p-4 bg-pixel-dark text-xs overflow-x-auto max-h-64 overflow-y-auto font-mono whitespace-pre-wrap text-pixel-cyan border border-pixel-green">{repair.diff}</pre>
                  </details>
                )}
              </div>
            )}
              ...
              <div>
                ...
                <div className="space-y-3">
                  {result.violations.map((v, i) => {
                    const st = IMPACT_STYLES[v.impact] || { badge:'text-pixel-gray border-gray-500', border:'border-l-4 border-gray-500' };
                    return (
                      <div key={i} className={`pixel-box-dark p-4 ${st.border}`}>
                        <div className="flex items-start justify-between gap-3 mb-2">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className={`font-pixel text-xs uppercase px-2 py-0.5 bg-pixel-dark border ${st.badge}`}>{v.impact}</span>
                            <span className="text-pixel-gray text-xs">{v.wcagCriteria || v.id}</span>
                          </div>
                          {v.helpUrl && <a href={v.helpUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-pixel-cyan hover:underline shrink-0">Learn more ↗</a>}
                        </div>
                        <div className="text-pixel-white text-xs font-semibold mb-1">{v.help || v.description}</div>
